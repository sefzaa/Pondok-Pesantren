...
import SidebarPerizinan from '@/components/SidebarPerizinan'; // Menggunakan sidebar baru

export default function PerizinanLayout({ children }) {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const pathname = usePathname();

  // Logika untuk membuat judul dinamis sesuai URL
  const pageTitle = useMemo(() => {
    if (pathname === '/perizinan') {
      return 'Dashboard';
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="fixed inset-0 flex z-40">
                <Transition.Child as={Fragment} enter="transition ease-in-out duration-300 transform" enterFrom="-translate-x-full" enterTo="translate-x-0" leave="transition ease-in-out duration-300 transform" leaveFrom="translate-x-0" leaveTo="-translate-x-full">
                    <Dialog.Panel className="relative flex-1 flex flex-col max-w-xs w-full bg-white">
                        ...
                        <SidebarPerizinan setSidebarOpen={setSidebarOpen} />
                    </Dialog.Panel>
                </Transition.Child>
      ...
      {/* ▼▼▼ PERUBAHAN 1: Menambahkan shadow pada container sidebar desktop ▼▼▼ */}
      ...
      <div className="lg:pl-64 flex flex-col flex-1">
        ...
        <header className="sticky top-0 z-10 flex-shrink-0 flex h-16 bg-white shadow-sm items-center justify-between px-4 sm:px-6 lg:px-8">
          {/* ▼▼▼ PERUBAHAN 3: Menghapus `border-r` dari tombol menu mobile ▼▼▼ */}
          <button
            type="button"
          ...
          {/* Menambahkan `ml-4 lg:ml-0` untuk memberi jarak pengganti border */}
