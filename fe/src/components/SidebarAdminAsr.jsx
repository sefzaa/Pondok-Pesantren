...
const menuItems = [
  {
    group: "Main Menu",
    items: [
      { name: "Dashboard", icon: FiHome, href: "/adminAsr" },
      {
        name: "Kegiatan Rutin",
        icon: FiCalendar,
        href: "/adminAsr/kegiatan-rutin",
      },
      {
        name: "Kegiatan Tambahan",
        icon: FiCalendar,
        href: "/adminAsr/kegiatan-tambahan",
      },
      {
        name: "Pelanggaran",
        icon: FiAlertTriangle,
        href: "/adminAsr/pelanggaran",
      },
    ],
  },
  {
    ...
    items: [
      { name: "Santri", icon: FiUsers, href: "/adminAsr/santri" },
      { name: "Kamar", icon: FiGrid, href: "/adminAsr/kamar" },
    ],
  },
  {
    group: "Manajemen Akun",
    items: [
      { name: "Wali Kamar", icon: FiUserCheck, href: "/adminAsr/wali-kamar" },
      { name: "Div. Perizinan", icon: FiBox, href: "/adminAsr/div-perizinan" },
      {
        name: "Div. Inventaris",
        icon: FiBriefcase,
        href: "/adminAsr/div-inventaris",
      },
      { name: "Direktur", icon: FiBriefcase, href: "/adminAsr/direktur" },
    ],
  },
];
...
export default function SidebarAdminAsr({ setSidebarOpen }) {
  ...
  return (
    // ▼▼▼ PERUBAHAN 1: Hapus `border-r` dari sini ▼▼▼
    // Shadow sudah ditangani oleh parent component di DashboardLayout.js
    <aside className="bg-white w-full h-full flex flex-col">
      {/* ▼▼▼ PERUBAHAN 2: Ganti `border-b` menjadi `shadow-sm` ▼▼▼ */}
        ...
        <Menu as="div" className="relative">
          <Menu.Button className="w-full flex items-center gap-3 text-left p-2 rounded-lg hover:bg-gray-100 transition-colors">
            ...
            <div className="flex-1">
              <p className="font-semibold text-sm text-gray-800">Nama User</p>
